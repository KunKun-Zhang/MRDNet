...
def differential_validation(net, loss_fn, data_loader, device):
    ...
    for batch_ind, data in enumerate(data_loader):
        ...
        for _ in range(occur_num):
            ...
            out_argmax = torch.max(out, dim=1)[1]
            one_acc = torch.eq(out_argmax, item_label)
            iter_sum += (torch.sum(one_acc).detach().item()) / one_acc.shape[0]
            sub_error = torch.abs(out_argmax - item_label)
            temp_error = torch.min(sub_error, 360 - sub_error)
            ...
            iter_error += torch.sum(temp_error).detach().item() / temp_error.shape[0]

            check_a = torch.ones(size=temp_error.shape, device=device)
            check_b = torch.zeros(size=temp_error.shape, device=device)
            bias_acc = torch.where(temp_error < 3, check_a, check_b)
            iter_bias_sum += torch.sum(bias_acc).detach().item() / bias_acc.shape[0]

        iter_sum /= occur_num
        iter_loss /= occur_num
        iter_error /= occur_num
        iter_bias_sum /= occur_num

        iteration_sum += iter_sum
        iteration_loss += iter_loss
        iteration_error += iter_error
        iteration_bias_sum += iter_bias_sum
...
def train_loop(net, opt, loss_fn):
    ...
    print("train")
    epochs = 2000
    path = '/home/work3/DOA/save/fix_array/0328'
