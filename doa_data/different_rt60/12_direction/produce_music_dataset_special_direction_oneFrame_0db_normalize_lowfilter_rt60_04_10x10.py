...
def create_room_box(direction, file_path, m=10, n=10, c=343.0, fs=16000, nfft=512, room_dim=np.r_[10.0, 8.0],
    ...
    e_absorption, max_order = pra.inverse_sabine(0.4, room_dim)
...
def main(one_path1, one_path2):
    ...
    file_list = []
    for root, dirs, files in os.walk(one_path1):
        # root = root.replace('\\', '/')
        for file in files:
...
if __name__ == '__main__':
    ...
    all_time = time.time()
    in_path = '/home/work3/DOA_data/train-clean-100/LibriSpeech/train-clean-100'
    out_path = '/home/ssd02/12_direction_oneFileAllDirection_onlySingleFrame_11000_0db_normalize_lowfilter_rt60_04_mic10x10/train'
    print('#####################################################')
    print(in_path)
    print('------------------------->')
    print(out_path)
